...
#include "collin/net/openssl_socket_service.hpp"
		...
		class basic_stream_ssl_socket : public basic_stream_socket<Protocol>
		{
			...
			public:
				...
				explicit basic_stream_ssl_socket(net::io_context& ctx)
					: base{ctx}, ssl{SSL_new(collin::net::use_service<collin::net::openssl_socket_service>(ctx).get_context().get_context())}
				{
				}

				basic_stream_ssl_socket(net::io_context& ctx, const protocol_type& procotol)
					: base{ctx, procotol}, ssl{SSL_new(collin::net::use_service<collin::net::openssl_socket_service>(ctx).get_context().get_context())}
				{
					SSL_set_fd(ssl, native_handle());
				}

				basic_stream_ssl_socket(net::io_context& ctx, const endpoint_type& endpoint)
					: base{ctx, endpoin}, ssl{SSL_new(collin::net::use_service<collin::net::openssl_socket_service>(ctx).get_context().get_context())}
				{
					SSL_set_fd(ssl, native_handle());
				}

				basic_stream_ssl_socket(net::io_context& ctx, const protocol_type& protocol, const native_handle_type& handle)
					: base{ctx, protocol, handle}, ssl{SSL_new(collin::net::use_service<collin::net::openssl_socket_service>(ctx).get_context().get_context())}
				{
					SSL_set_fd(ssl, native_handle());
				}
				...
				{
					connect(endpoint, collin::throw_on_error{"basic_stream_ssl_socket::connect"});
				}
		...
		class basic_socket_ssl_streambuf : public std::basic_streambuf<char>
		{
			...
			public:
				...
				basic_socket_ssl_streambuf(basic_socket_ssl_streambuf&& rhs) noexcept
					: base{std::move(static_cast<base&>(static_rhs))}, socket_{std::move(rhs.socket_)}
				{
					set_buffers();
				}
