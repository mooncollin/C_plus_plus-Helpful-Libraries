...
namespace collin
{
	namespace multiprecision
	{
		...
		class big_int
		{
			...
			template<std::size_t min_precision, signed_type S>
				requires(collin::platform::uint_trivial_type<min_precision>)
			struct trivial_type_t<min_precision, S>
			{
				using type = std::conditional_t<S == signed_type::signed_magnitude, 
									collin::platform::least_bits_int<min_precision>,
									collin::platform::least_bits_unsigned_int<min_precision>>;
			};
			...
			public:
				...
				template<collin::concepts::integral I>
				constexpr big_int& operator+=(I val) noexcept(noexcept(add(other)) || noexcept(other.subtract(*this)) || noexcept(subtract(other)))
				{
					if (val < 0)
					{
						subtract(-val);
		...
		namespace details
		{
			...
			template<std::size_t MinBits, signed_type SignType, checked_type Checked, class Allocator, collin::concepts::integral I>
			struct big_int_supertype<big_int<MinBits, SignType, Checked, Allocator>, I>
			{
				using first = big_int<MinBits, SignType, Checked, Allocator>;
				using second = I;

				static constexpr std::size_t bits_used = MinBits ? std::max(MinBits, collin::platform::bits_in_type<I>)
																 : 0;
...
namespace std
{
	template<std::size_t MinBits, collin::multiprecision::signed_type SignType, collin::multiprecision::checked_type Checked, class Allocator>
	constexpr void swap(collin::multiprecision::big_int<MinBits, SignType, Checked, Allocator>& lhs, collin::multiprecision::big_int<MinBits, SignType, Checked, Allocator>& rhs) noexcept
	{
		lhs.swap(rhs);
	}
}