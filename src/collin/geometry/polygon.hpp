	...
	namespace geometry
	{
		...
		class polygon
		{
			public:
				...
				template<class... Sides>
					requires(collin::minimum_static_initializer_list<rep, 3, Sides...>::value)
				...
				template<std::size_t I>
					requires(I <= S && I != 0)
				[[nodiscard]] constexpr rep side() const noexcept
				{
					return sides_[I - 1];
				}
		...
		class regular_polygon
		{
			public:
				...
				template<class Rep2>
				[[nodiscard]] friend constexpr bool operator==(const regular_polygon& lhs, const regular_polygon<Rep2, S>& rhs) noexcept
				{
		...
		[[nodiscard]] constexpr angle sum_interior_angles(std::size_t num_sides) noexcept
		{
			return angle{{num_sides - 2, 1}, angle_type::radian};
		}
		...
		{
			return sum_interior_angles(S) / S;
		}
		...
		{
			return 0.25L * S * p.side() * p.side() * collin::math::cot(std::numbers::pi_v<long double> / S);
		}
