	...
	namespace geometry
	{
		...
		class triangle : public polygon<Rep, 3>
		{
			public:
				...
				{
					return collin::geometry::get_sides(lhs) == collin::geometry::get_sides(rhs) &&
						   collin::geometry::get_angles(lhs) == collin::geometry::get_angles(rhs);
				}
		...
		[[nodiscard]] constexpr auto get_angle(const triangle<Rep>& t) noexcept
		{
			...
			return collin::geometry::acos(sides_calculation);
		}
		...
		{
			const auto first_proportion = collin::math::rational{lhs.side<1>() * lhs.side<2>(), rhs.side<1>() * rhs.side<2>()};
			const auto second_proportion = collin::math::rational{lhs.side<2>() * lhs.side<3>(), rhs.side<2>() * rhs.side<3>()};
			const auto third_proportion = collin::math::rational{lhs.side<3>() * lhs.side<1>(), rhs.side<3>() * rhs.side<1>()};
