...
namespace cmoon
{
	namespace cuda
	{
		template<class T>
		__global__
		void matrix_multiply_kernel(int m, int n, int k, const T* const input1, const T* const input2, T* out)
		{
		...
		template<class T>
		__global__
		void square_matrix_multiply_kernel(int n, const T* const input1, const T* const input2, T* out)
		{
