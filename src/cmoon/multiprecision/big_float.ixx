export module cmoon.multiprecision.big_float;
...
import cmoon.multiprecision.properties;
...
namespace cmoon::multiprecision
{
	...
	class big_float
	{
		...
		template<std::size_t Digits>
			requires(cmoon::float_trivial_type<Digits> && Digits != 0)
		struct trivial_type_t<Digits>
		{
			using type = cmoon::least_digits_float<Digits>;
		};
		...
		private:
			...
			constexpr void assign_float(F val)
			{
				...
				else
				{
					...
					{
						cmoon::basic_rational<signed_double_limb_type> r{cmoon::abs(val)};
			...
			template<std::size_t AmountDigits2, checked_type Checked2, class Allocator2>
			[[nodiscard]] constexpr std::strong_ordering compare_absolute(const big_float<AmountDigits2, Checked2, Allocator2>& rhs) const noexcept
				requires(!trivial && !big_float<AmountDigits2, Checked2, Allocator2>::trivial)
			{
				if (is_special() || rhs.is_special())
				{
					if (is_infinity())
					{
						return rhs.is_infinity() ? std::strong_ordering::equal : std::strong_ordering::greater;
					}
					...
					{
						return rhs.is_zero() ? std::strong_ordering::equal : std::strong_ordering::less;
					}
					else if (rhs.is_zero())
					{
				...
				return cmoon::sequence_compare(begin(), end(), rhs.begin(), rhs.end());
			}
			...
			constexpr void divide(const big_float<AmountDigits2, Checked2, Allocator2>& other)
			{
				...
				else
				{
					...
					const auto shift_even = cmoon::is_even(shift);
					if constexpr (fixed_precision)
					{
	...
	namespace details
	{
		...
		template<std::size_t AmountDigits, checked_type Checked, class Allocator, std::floating_point F>
		...
		template<std::size_t AmountDigits, checked_type Checked, class Allocator, std::integral I>
	...
	template<std::size_t AmountDigits, checked_type Checked, class Allocator, cmoon::arithmetic A>
