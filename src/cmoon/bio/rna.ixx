...
namespace cmoon
{
	namespace bio
	{
		...
		std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const rna<Allocator>& s)
		{
		...
		template<class CharT, class Traits, class Allocator>
		std::basic_istream<CharT, Traits>& operator>>(std::basic_istream<CharT, Traits>& is, rna<Allocator>& s)
		{
			CharT ch;
			s.sequence().clear();
			is >> std::ws;
...
namespace cmoon
{
	...
	struct formatter<rna<Allocator>> : public formatter<std::basic_string_view<CharT, CharT>>
	{
		template<class OutputIt>
		auto format(const rna<Allocator>& r, basic_format_context<OutputIt, CharT>& ctx)
		{
