        ...
        namespace ip
        {
            ...
            class address_v6
            {
                public:
                    ...
                    [[nodiscard]] friend constexpr bool operator==(const address_v6& a, const address_v6& b) noexcept = default;
                    [[nodiscard]] friend constexpr bool operator!=(const address_v6& a, const address_v6& b) noexcept = default;

                    [[nodiscard]] friend constexpr std::strong_ordering operator<=>(const address_v6& a, const address_v6& b) noexcept
                    {
                        for (std::size_t i = 0; i < std::size(a.address_); i++)
                        {
                            const auto r = a.address_[i] <=> b.address_[i];
            ...
            class network_v4
            {
                public:
                    ...
                    constexpr network_v4(const address_v4& addr, const address_v4& mask)
                        : addr_{addr}, prefix_len_{std::popcount(mask.to_uint())}
                    {
                        if (prefix_len_ != 0)
                        {
                            if ((mask.to_uint() & 0x80000000) == 0)
                            {
            ...
            class basic_endpoint
            {
                ...
                public:
                    ...
                    {
                        return std::visit(cmoon::overloaded {
...
{
    template<>
    struct hash<cmoon::net::ip::address>
    {
        using result_type = std::size_t;
        using argument_type = cmoon::net::ip::address;
        ...
        std::size_t operator()(const argument_type& a) const noexcept
        {
            if (a.is_v4())
            {
                ...
                return cmoon::hash_bytes(&a4, sizeof(a4), 0xc70f6907UL);
            }
            else
            {
                const auto a6 = a.to_v6();
                return cmoon::hash_bytes(&a6, sizeof(a6), 0xc70f6907UL);
            }
        }
    };

    template<>
    struct hash<cmoon::net::ip::address_v4>
    {
        using result_type = std::size_t;
        using argument_type = cmoon::net::ip::address_v4;

        std::size_t operator()(const argument_type& a) const noexcept
        {
            const auto b = a.to_bytes();
            return cmoon::hash_bytes(b.data(), sizeof(b), 0xc70f6907UL);
        }
    };

    template<>
    struct hash<cmoon::net::ip::address_v6>
    {
        using result_type = std::size_t;
        using argument_type = cmoon::net::ip::address_v6;
