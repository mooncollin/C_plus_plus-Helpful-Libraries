...
namespace cmoon
{
	...
	class basic_format_arg
	{
		...
		private:
			...
			template<details::context_has_formatter_type<Context> T>
			...
			friend struct details::format_arg_store;
	...
	template<class Visitor, class Ctx>
	auto visit_format_arg(Visitor&& vis, basic_format_arg<Ctx> arg)
	{
	...
	class basic_format_args
	{
		public:
			...
			template<class... Args>
			basic_format_args(const details::format_arg_store<Context, Args...>& store) noexcept
				: data_{store.args} {}
	...
	struct formatter<std::basic_string_view<CharT, Traits>, CharT> : public details::base_formatter<std::basic_string_view<CharT, Traits>, CharT>
	{
		template<class OutputIt>
		auto format(std::basic_string_view<CharT, Traits> str, basic_format_context<OutputIt, CharT>& ctx)
		{
			return details::write_string_view(str, ctx, this->parser);
		}
	};

	template<typename CharT>
	struct formatter<CharT, CharT> : public details::base_formatter<CharT, CharT>
	{
		template<class OutputIt>
		auto format(CharT c, basic_format_context<OutputIt, CharT>& ctx)
		{
			return details::write_string_view(std::basic_string_view<CharT>{std::addressof(c), 1}, ctx, this->parser);
		}
	};

	template<>
	struct formatter<char, wchar_t> : public details::base_formatter<char, wchar_t>
	{
		template<class OutputIt>
		auto format(char c, basic_format_context<OutputIt, wchar_t>& ctx)
		{
			...
			return details::write_string_view(std::wstring_view{std::addressof(wide), 1}, ctx, this->parser);
		}
	};
	...
	template<typename CharT>
	struct formatter<int, CharT> : public details::base_formatter<int, CharT>
	{
		template<class OutputIt>
		auto format(int value, basic_format_context<OutputIt, CharT>& ctx)
		{
			return details::write_integer(value, ctx, this->parser);
		}
	};

	template<typename CharT>
	struct formatter<short, CharT> : public details::base_formatter<short, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<unsigned short, CharT> : public details::base_formatter<unsigned short, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<unsigned int, CharT> : public details::base_formatter<unsigned int, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<long, CharT> : public details::base_formatter<long, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<unsigned long, CharT> : public details::base_formatter<unsigned long, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<long long, CharT> : public details::base_formatter<long long, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<unsigned long long, CharT> : public details::base_formatter<unsigned long long, CharT>
	{
		template<class OutputIt>
	...
	template<typename CharT>
	struct formatter<bool, CharT> : public details::base_formatter<bool, CharT>
	{
		template<class OutputIt>
		auto format(bool value, basic_format_context<OutputIt, CharT>& ctx)
		{
			return details::write_boolean(value, ctx, this->parser);
		}
	};

	template<typename CharT>
	struct formatter<const void*, CharT> : public details::base_formatter<const void*, CharT>
	{
		template<class OutputIt>
		auto format(const void* value, basic_format_context<OutputIt, CharT>& ctx)
		{
			return details::write_pointer(value, ctx, this->parser);
		}
	};
	...
	template<typename CharT>
	struct formatter<double, CharT> : public details::base_formatter<double, CharT>
	{
		template<class OutputIt>
		auto format(double value, basic_format_context<OutputIt, CharT>& ctx)
		{
			return details::write_floating(value, ctx, this->parser);
		}
	};

	template<typename CharT>
	struct formatter<long double, CharT> : public details::base_formatter<long double, CharT>
	{
		template<class OutputIt>
	...
	struct formatter<std::chrono::duration<Rep, Period>, CharT> : public details::base_chrono_formatter<std::chrono::duration<Rep, Period>, CharT>
	{
		...
		public:
			...
			{
				const auto overall_seconds = std::chrono::duration_cast<std::chrono::duration<Rep>>(d).count();
				const auto overall_minutes = overall_seconds / 60;
				const auto hours = overall_minutes / 60;
				const std::tm time {.tm_sec = static_cast<int>(overall_seconds % 60),
									.tm_min = static_cast<int>(overall_minutes % 60),
									.tm_hour = static_cast<int>(hours)};
	...
	template<class Ctx, class... Args>
	details::format_arg_store<Ctx, Args...> make_format_args(const Args&... args)
	{
		return {args...};
	}

	template<class... Args>
	details::format_arg_store<wformat_context, Args...> make_wformat_args(const Args&... args)
	{
		return {args...};
	}
	...
	namespace details
	{
		...
		Out vformat_to(Out out, const std::locale& loc, std::basic_string_view<CharT> fmt, format_args_t<Out, CharT>&& args)
		{
			...
			while (!details::at_end(parse_context))
			{
				const auto ch = details::get_char(parse_context);
				if (in_open_curly)
				{
					if (ch == open_curly)
					{
						details::set_char(format_context, open_curly);
						details::advance_one(format_context);
						details::advance_one(parse_context);
					}
					else
					{
						const auto id = details::parse_arg_id(parse_context);
						if (details::get_char(parse_context) == colon)
						{
							details::advance_one(parse_context);
						}
						...
						if (details::at_end(parse_context))
						{
							throw format_error{"invalid format string"};
						}
						else
				...
				else if (in_close_curly)
				{
					if (ch == close_curly)
					{
						details::set_char(format_context, close_curly);
						details::advance_one(format_context);
						details::advance_one(parse_context);
					}
					else
				...
				{
					details::set_char(format_context, ch);
					details::advance_one(format_context);
					details::advance_one(parse_context);
				}
			}
	...
	{
		details::format_to_n_tracker tracker{out, n};
	...
	{
		details::format_size_counter<char> c;
	...
	{
		details::format_size_counter<wchar_t> c;
		c = format_to(c, l, fmt, args...);
		return c.count();
	}
