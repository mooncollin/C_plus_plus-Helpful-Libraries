...
namespace cmoon
{
	namespace windows
	{
		...
		class message_box_option
		{
			public:
				template<class... T>
					requires((details::is_message_box_option<T> && ...))
				constexpr message_box_option(T... options) noexcept
					: value_{(static_cast<std::underlying_type_t<T>>(options) | ...)} {}

				template<class... T>
					requires((details::is_message_box_option<T> && ...) && sizeof...(T) > 0)
		...
		message_box_result show_message_box(::HWND owner, std::string_view message, std::string_view title, message_box_option options)
		{
			return message_box_result{::MessageBoxA(owner, message.data(), title.data(), options.value())};
		}

		message_box_result show_message_box(::HWND owner, std::wstring_view message, std::wstring_view title, message_box_option options)
		{
			return message_box_result{::MessageBoxW(owner, message.data(), title.data(), options.value())};
		}

		message_box_result show_message_box(std::string_view message, std::string_view title, message_box_option options)
		{
			return show_message_box(nullptr, message, title, options);
		}

		message_box_result show_message_box(std::wstring_view message, std::wstring_view title, message_box_option options)
		{
			return show_message_box(nullptr, message, title, options);
		}
	}
