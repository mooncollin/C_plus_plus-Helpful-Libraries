...
namespace cmoon::execution
{
	...
	struct upon_done_t
	{
		...
		public:
			...
			{
				return upon_done_adapter<F>{std::forward<F>(f)};
			}
	};
	...
	{
		public:
			constexpr upon_done_adapter(F&& f)
				: f_{std::forward<F>(f)} {}

			template<sender S>
			constexpr friend auto operator|(S&& s, upon_done_adapter&& a)
			{
				return upon_done(std::forward<S>(s), std::move(a.f_));
			}
		private:
			F f_;
	};
