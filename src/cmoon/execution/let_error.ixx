...
namespace cmoon::execution
{
	...
	{
		public:
			constexpr let_error_receiver(F&& f, R&& r)
				: f_{std::forward<F>(f)}, out_r{std::forward<R>(r)} {}

			template<class... Args>
			constexpr friend void tag_invoke(set_value_t, let_error_receiver&& r, Args&&... args)
			{
				execution::set_value(std::move(r.out_r), std::forward<Args>(args)...);
			}

			template<class E>
			constexpr friend void tag_invoke(set_error_t, let_error_receiver&& r, E&& e) noexcept
			{
				try
				{
					execution::start(
			...
			constexpr friend void tag_invoke(set_done_t, let_error_receiver&& r) noexcept
			{
	...
	struct let_error_sender : public sender_base
	{
		public:
			...
			template<receiver R>
			constexpr friend auto tag_invoke(connect_t, let_error_sender&& s, R&& out_r)
			{
				return execution::connect(std::move(s.s_),
										  let_error_receiver<F, R>{std::move(s.f_), std::forward<R>(out_r)});
			}
		private:
			S s_;
			F f_;
	...
	struct let_error_t
	{
		...
		public:
			...
			{
				return let_error_adapter<F>{std::forward<F>(f)};
			}
	};
	...
	{
		public:
			constexpr let_error_adapter(F&& f)
				: f_{std::forward<F>(f)} {}

			template<sender S>
			constexpr friend auto operator|(S&& s, let_error_adapter&& a)
			{
				return let_error(std::forward<S>(s), std::move(a.f_));
			}
		private:
			F f_;
	};
