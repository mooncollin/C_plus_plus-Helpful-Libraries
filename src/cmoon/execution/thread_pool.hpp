	...
	namespace execution
	{
		class static_thread_pool
		{
			private:
				...
				{
					public:
						Base<execution::blocking_t::never_t,
							 Relationship,
							 Outstanding,
							 AllocatorType>
						require(const execution::blocking_t::never_t) const
						{
							return {*this};
						}

						Base<execution::blocking_t::possibly_t,
							 Relationship,
							 Outstanding,
							 AllocatorType>
						require(const execution::blocking_t::possibly_t) const
						{
							return {*this};
						}

						Base<execution::blocking_t::always_t,
							 Relationship,
							 Outstanding,
							 AllocatorType>
						...
						Base<Blocking,
							 ...
							 Outstanding,
							 AllocatorType>
						require(const execution::relationship_t::continuation_t) const
						{
							return {*this};
						}
						...
						Base<Blocking,
							 ...
							 Outstanding,
							 AllocatorType>
						require(const execution::relationship_t::fork_t) const
						{
							return {*this};
						}
						...
						Base<Blocking,
							 ...
							 AllocatorType>
						require(const execution::outstanding_work_t::tracked_t) const
						{
							return {*this};
						}
						...
						Base<Blocking,
							 ...
							 AllocatorType>
						require(const execution::outstanding_work_t::untracked_t) const
						{
							return {*this};
						}
						...
						Base<Blocking,
							 ...
							 Outstanding,
							 ProtoAllocator>
						require(const execution::allocator_t<ProtoAllocator>& other) const
						{
							return {*this, other.value()};
						}

						auto require(const execution::allocator_t<void>&) const
						{
							return require(execution::allocator(std::allocator<void>{}));
						}

						[[nodiscard]] static constexpr execution::bulk_guarentee_t query(const execution::bulk_guarentee_t) noexcept
						{
							return execution::bulk_guarentee.parallel;
						}

						[[nodiscard]] static constexpr execution::mapping_t query(const execution::mapping_t) noexcept
						{
							return execution::mapping.thread;
						}

						[[nodiscard]] static constexpr execution::blocking_t query(const execution::blocking_t) noexcept
						{
							return Blocking{};
						}

						[[nodiscard]] static constexpr execution::relationship_t query(const execution::relationship_t) noexcept
						{
							return Relationship{};
						}

						[[nodiscard]] static constexpr execution::outstanding_work_t query(const execution::outstanding_work_t) noexcept
						{
							return Outstanding{};
						}

						auto query(const execution::allocator_t<void>&) const noexcept
						{
							return std::allocator<void>{};
						}

						template<class ProtoAllocator>
						auto query(const execution::allocator_t<ProtoAllocator>&) const noexcept
						{
							return alloc;
						}
					protected:
															 ...
															 AllocatorType>
				{
					...
					private:
						static constexpr bool is_blocking = std::same_as<Blocking, execution::blocking_t::always_t>;
						static constexpr bool is_tracking = std::same_as<Outstanding, execution::outstanding_work_t::tracked_t>;
				...
				class scheduler_impl
				{
									...
									void start()
									{
										...
										exec.execute([func = std::move(r)] {
											try
											{
												...
												{
													func();
													execution::set_done(std::move(func));
												}
												else
												{
													auto&& v = func();
													execution::set_value(std::move(func), std::move(v));
												}
											}
											catch (...)
											{
												execution::set_error(std::move(func), std::current_exception());
											}
										});
									}
					...
					public:
						...
						//static_assert(execution::typed_sender<sender_type>);
						...
						scheduler_impl<OtherAllocatorType> require(const execution::allocator_t<OtherAllocatorType>& a) const
						{
							return {*this, a.value()};
						}

						auto require(const execution::allocator_t<void>& a) const
						{
							return require(execution::allocator(std::allocator<void>{}));
						}

						static_thread_pool& query(const execution::context_t) const noexcept
						{
							return pool;
						}
						...
						template<class OtherAllocatorType>
						auto query(const execution::allocator_t<OtherAllocatorType>&) const noexcept
						{
							return alloc;
						}
