...
namespace cmoon::execution
{
	...
	struct let_done_t
	{
		...
		public:
			...
			{
				return let_done_adapter<F>{std::forward<F>(f)};
			}
	};
	...
	{
		public:
			constexpr let_done_adapter(F&& f)
				: f_{std::forward<F>(f)} {}

			template<sender S>
			constexpr friend auto operator|(S&& s, let_done_adapter&& a)
			{
				return let_done(std::forward<S>(s), std::move(a.f_));
			}
		private:
			F f_;
	};
