...
namespace collin
{
    namespace strings
    {
        ...
        {
            return algorithms::any_of(container, is_vowel);
        }

        template<class Container>
        bool contains_consonant(const Container& container)
        {
            return algorithms::any_of(container, is_consonant);
        }

        template<class Container>
        void remove_punctuation(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::ispunct));
        }

        template<class Container>
        void remove_spaces(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isspace));
        }

        template<class Container>
        void remove_alphanumerics(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isalnum));
        }

        template<class Container>
        void remove_blanks(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isblank));
        }

        template<class Container>
        void remove_cntrls(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::iscntrl));
        }

        template<class Container>
        void remove_graphicals(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isgraph));
        }

        template<class Container>
        void remove_printables(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isprint));
        }

        template<class Container>
        void remove_hexdigits(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isxdigit));
        }

        template<class Container>
        void remove_upper(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::isupper));
        }

        template<class Container>
        void remove_lower(Container& container)
        {
            algorithms::erase_if(container, static_cast<int(*)(int)>(std::islower));
        }
        ...
        void remove_character(Container& container, char c)
        {
            ...
            algorithms::erase_if(container, f);
        }

        template<class Container>
        void uppercase(Container& container)
        {
            algorithms::transform(container, static_cast<int(*)(int)>(std::toupper));
        }

        template<class Container>
        void lowercase(Container& container)
        {
            algorithms::transform(container, static_cast<int(*)(int)>(std::tolower));
        }
